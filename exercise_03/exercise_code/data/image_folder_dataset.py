...
class ImageFolderDataset(Dataset):
    ...
    def make_dataset(directory, class_to_idx):
        ...
        for cls, i in class_to_idx.items():
            ...
            for f in os.scandir(cls_path):
                if f.is_file() and f.name.endswith('.png'):
                    images.append(os.path.join(cls_path, f.name))
                    labels.append(i)
...
class RescaleTransform:
    ...
    def __call__(self, images):
        ...
        curr_range = self._data_max - self._data_min
        tgt_range = self.max - self.min
        forward_r, backward_r = tgt_range / curr_range, curr_range / tgt_range
        intermediate_min = self.min * backward_r

        images += (intermediate_min - self._data_min)
        images *= forward_r
