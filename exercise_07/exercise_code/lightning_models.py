...
class TwoLayerNet(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            self.visualize_predictions(images, out, targets)
    ...
    def visualize_predictions(self, images, preds, targets):
        ...
        num_rows = torch.tensor(len(images), device='cpu').float().sqrt().floor()
    ...
    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.hparams["batch_size"], num_workers=1)
